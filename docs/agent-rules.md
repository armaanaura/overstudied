...
7. Do not duplicate article content for domain pages. Every article must assign `Article.category` to one valid domain slug from `ArticleCategory`; let the domain route filter canonical article data.
